...
export class DriveState {
    ...
    @Selector()
    static getUserId(state: DriveStateModel) {
        return state.remoteUser.user_id
    }

    @Selector()
    static getAccessToken(state: DriveStateModel) {
        return state.remoteUser.access_token
    }
    ...
    @Selector()
    static isConnect(state: DriveStateModel) {
    ...
    @Action (LoadRemoteUser)
    loadRemoteUser(ctx: StateContext<DriveStateModel>, action: LoadRemoteUser) {
        ctx.patchState({
            remoteUser: action.user
        })
    }
    ...
    @Action(IsConnect)
    IsConnect(ctx: StateContext<DriveStateModel>) {
        ctx.patchState({
            isConnect: !ctx.getState().isConnect
        })
    }
